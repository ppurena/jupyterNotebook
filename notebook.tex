...
\tightlist
\item
  \textbf{La Aplicación Web de Notebook}: una aplicación web interactiva
  donde se crean los documentos ``notebook''.
\item
  ...
  notebook.
\item
  \textbf{Documentos Notebook}: archivo con una representación de todo
  el contenido visible en la aplicación web del notebook, incluidas las
  entradas y salidas de los cálculos, el texto, las ecuaciones, las
  imágenes y las representaciones multimedia de objetos. \emph{Cada
  documento Notebook tiene su propio kernel}.
\end{itemize}

    \hypertarget{para-que-puede-servir-los-notebooks}{%
\section{¿Para que puede servir los
Notebooks?}\label{para-que-puede-servir-los-notebooks}}
...
{\color{incolor}In [{\color{incolor}1}]:} \PY{k}{def} \PY{n+nf}{sum}\PY{p}{(}\PY{n}{a}\PY{p}{,}\PY{n}{b}\PY{p}{)}\PY{p}{:}
        ...
        \PY{n+nb}{print}\PY{p}{(}\PY{n+nb}{sum}\PY{p}{(}\PY{l+m+mi}{1}\PY{p}{,}\PY{l+m+mi}{6}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
